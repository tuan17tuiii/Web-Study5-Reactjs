...
function ExamDetai() {
  ...
  const { findAll, findById } = Examdata()
  const { findByIdUserfull } = User_scoreService()
  const { id } = useParams();
  useEffect(() => {
    async function loadExams() {
      let dataExam = await findById(Number(id)); // Giả sử fetchExams là hàm trả về Promise
      ...
      dataExamed?.forEach((x: any) => {
        console.log("dataxx", x.id_exam.id
          , id); if (x.id_exam.id === Number(id)) {
            setUser_exam(x)
